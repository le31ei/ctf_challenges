...
$query=$db->findall("510_newsclass");
...
$db->delete("510_newbase","`id` in (".$ID_Dele.")");
$db->delete("510_newscontent","`nid` in (".$ID_Dele.")");
$db->get_admin_msg("news_list.php","删除成功");
}else{
$db->delete("510_newbase","`id` = $_GET[del]");
$db->delete("510_newscontent","`nid` = $_GET[del]");
$db->get_admin_msg("news_list.php","删除成功");
}
}
$cidd=$_GET[classid];
...
?>
    ...
    <tr align="center">
         ...
         <td class="td_bg" width="49%" height="26">新闻标题</td>
         <td class="td_bg" width="13%" height="">新闻分类</td>
	     <td class="td_bg" width="9%" height="26">作者;</td>
	     <td class="td_bg" width="11%" height="">日期</td>
		 <td class="td_bg" width="12%">操作</td>
		 <td class="td_bg" width="3%" height=""><input id=chkAll
...
if($cidd!=""){
$sql="SELECT * FROM `510_newbase` where `cid`=$cidd";
}else{
...
if($cidd!=""){
$SQL="SELECT * FROM `510_newbase` where `cid`=$cidd  limit $firstcount,$pagesize";
}else{
$SQL="SELECT * FROM `510_newbase` limit $firstcount,$pagesize";
}
...
<td class="td_bg" width="49%" height="26"><?php echo $row[title] ?></td>
<td class="td_bg" width="13%" height=""><?php echo $news_class_arr[$row[cid]] ?></td>
<td class="td_bg" width="9%" height="26"><?php echo $row[author] ?></td>
<td class="td_bg" width="11%" height=""><?php echo date("Y-m-d",$row[date_time]) ?></td>
<td class="td_bg" width="12%"><a href="news_edit.php?id=<?php echo $row[id] ?>">编辑</a> <a href="?del=<?php echo $row[id] ?>">删除</a></td>
<td class="td_bg" width="3%" height=""><input type=checkbox value=<?php echo $row[id] ?> name="lin[]" onClick=Checked(form)></td>
</tr>
<?php
 }
}
