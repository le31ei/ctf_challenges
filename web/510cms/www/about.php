...
$sm_comp[]=array("title"=>$row_comp[title],"id"=>$row_comp[id],"content"=>$row_comp[content]);
}
...
$query=$db->query("SELECT * FROM `510_single` where `type_id`=1 order by `sort` asc  limit 0,1");
$sm_content=$db->fetch_array($query);
$smarty->assign("sm_content",$sm_content);
}
